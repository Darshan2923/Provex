...
export const addWork = async (req, res, next) => {
    ...
    try {
        const saveProject = await newWork.save();
...
export const inviteProjectMember = async (req, res, next) => {
    ...
    for (let i = 0; i < project.members.length; i++) {
        if (project.members[i].id === req.user.id) {
            if (project.members[i].access === "Owner" || project.members[i].access === "Editor" || project.members[i].access === "Admin") {
                const mailOptions = {
    ...
    try {
        ...
        for (let i = 0; i < project.members.length; i++) {
            if (project.members[i].id == user.id) {
                return next(createError(403, "You are already a member in project"));
            }
        }
